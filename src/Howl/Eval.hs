...
transformMatchK eq k z = case eq of
  ...
  -- patterns. Variables that are present in p2 but not present in p1
  -- are bound to Sequence[] if p1 matches, and vice versa.
  SingleEq (PatAlt xs miss1 miss2 p1 p2) t ->
    let
      p1' = addNames xs p1
      p2' = addNames xs p2
    in
    k (MatchBranch (bindSeqVars miss1 Empty) (one $ SingleEq p1' t)) $
    k (MatchBranch (bindSeqVars miss2 Empty) (one $ SingleEq p2' t)) $
    z
          ...
          rule :<| rules -> do
            ...
            case result of
              Just transformedExpr
                | not (exprEqualFast transformedExpr expr') ->
                  -- Debug.traceShow (rule, pPrint expr', pPrint transformedExpr) $
                  pure result
              _ -> tryRulesSeq rules
