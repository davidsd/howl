...
-- | A newtype wrapper around Int. Equality checks are O(1).
...
-- | The Ord instance uses lexicographic ordering, so we need to look
-- up each symbol's associated name.
instance Ord Symbol where
  compare = comparing symbolShortText
...
symbolShortText :: Symbol -> TS.ShortText
symbolShortText (Symbol u) =
  unsafePerformIO $ do
  m <- HT.lookup reverseTable u
  pure (maybe (error "Couldn't find symbol Text") id m)

symbolName :: Symbol -> Text
symbolName = TS.toText . symbolShortText
